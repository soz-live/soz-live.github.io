---
layout: default
---

# Releases


## Contents

- Releases
  - [Introduction](#introduction)
  - [CORE](#core)
  - [BASE](#base)
  - [APPS](#apps)  
  - [ALL](#all)    
  - [DEV](#dev)
  - [Table](#table)
    

## Introduction

This page contains information on the different releases of **SOZ-LIVE**. Each of the Releases are listed below.
Refer to the Table below and the links to the included repos for further details.


## CORE

This release combines the core functionality of the **soz-live** repo with the **soz-files** repo to enable data files to be imported and exported from a drawing and objects and entities to be re-used in other drawings.  


## BASE

This release builds on the **CORE** release and adds in additional classes and functionality.  


## APPS

This release builds on the **BASE** release and adds in additional classes and functionality for AEC functionality. It also adds functionality for Rigging Items and Assemblies associated with Crane rigging arrangements.  


## ALL

This cumulative release includes the **APPS** release classes and functionality and adds the **soz-coders** repo the for support functions used by coders during development of the project.


## DEV

This release builds on the **ALL** release and adds in additional classes and functionality for Project Owners.  


## Table

Below is a table of repos vs releases, showing which repos are included with each release.  

For a list of functionality contained within each repo - click on the link.


{% include csvtable.html id=site.data.releases %}

Version:  1.30.5
<br>
Date: 20260531.1942
